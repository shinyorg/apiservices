...
public class NotificationBatchContext
{
    ...
    public async Task OnBatchStart(IEnumerable<PushRegistration> registrations)
    {
        ...
        await this.Wrap(x => x.OnBatchStart(this.batchId, list, this.notification)).ConfigureAwait(false);
    }
    ...
    public async Task OnBatchCompleted()
    {
        ...
        await this.Wrap(x => x.OnBatchCompleted(this.batchId, this.success, this.failures, this.notification)).ConfigureAwait(false);
    }
    ...
    public async Task OnNotificationSuccess(PushRegistration registration)
    {
        ...
        await this.Wrap(x => x.OnNotificationSuccess(this.batchId, registration, this.notification)).ConfigureAwait(false);
    }
